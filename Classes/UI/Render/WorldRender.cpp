...
bool WorldRender::init(const World *world, const UWRender* gameRender) {
    ...
    //init world objects
    for (World::ConstObjectIter iter = _world->getConstObjectIterBegin();
         iter != _world->getConstObjectIterBegin();
         ++iter) {
        createWorldObjectRender(iter->second);
    }
...
void WorldRender::addMapParticle(const MapParticleConfigData* particleData) {
    ...
    if (fileName.find(".csb") != string::npos) {
        effect = CocosUtils::playCSBAnimation(fileName, true, 0, nullptr);
    } else {
